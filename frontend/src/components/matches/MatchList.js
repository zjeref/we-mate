...
const MatchList = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${process.env.REACT_APP_API_URL}/user/matches`, { headers })
  ...
  const handleRematch = async () => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_URL}/user/leftswipe`,
        { targetUser: prefer[index].user._id },
        { headers }
      )
  ...
  const handleConnect = async () => {
    ...
    await axios
      .put(
        `${process.env.REACT_APP_API_URL}/user/rightswipe`,
        { targetUserId: prefer[index].user._id },
        { headers }
      )
  ...
  return (
    <div className="w-full h-[800px] max-w-6xl flex flex-col items-center p-2 ">
      {prefer && prefer.length > 0 && prefer.length > index ? (
        <div className=" bg-slate-400 w-full h-full flex justify-center  relative">
          <div className="w-full absolute top-4 flex justify-between px-10 z-20">
            <div>
              <button className="match-btn bg-white text-blackk">
                {prefer[index].score}% Match
              </button>
            </div>
            <div className="flex flex-col space-y-2">
              <button
                className="match-btn bg-primary text-white"
                onClick={handleRematch}
              >
                REMATCH
              </button>
              <button
                className="match-btn bg-white text-blackk"
          ...
          <img
            src={prefer[index].user.preference.photoes[0]}
            alt=""
            className="max-w-3xl h-auto object-cover filter brightness-100"
          />
